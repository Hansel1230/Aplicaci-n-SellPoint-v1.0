...
public class clsLnTiposEntidades {
	...
	public void Cargar(ref clsBeTbUsuarios oBeTiposEntidades, ref DataRow dr) {
		try {
				...
				oBeTiposEntidades.Statuss = ( IsDBNull(dr.Item['Statuss']) ? '' : dr.Item['Statuss'] )
				oBeTiposEntidades.NoEliminable = ( IsDBNull(dr.Item['NoEliminable']) ? False : dr.Item['NoEliminable'] )
				oBeTiposEntidades.FechaRegistro = ( IsDBNull(dr.Item['FechaRegistro']) ?  : dr.Item['FechaRegistro'] )
		catch (Exception ex) {
			throw ex;
		}
	}
	...
	public int Insertar(ref clsBeTiposEntidades oBeTiposEntidades) {
		try {
			...
			cmd.Parameters.Add(new SqlClient.SqlParameter("@STATUSS", oBeTiposEntidades.Statuss));
