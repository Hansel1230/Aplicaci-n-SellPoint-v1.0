...
public class clsLnGruposEntidades {
	...
	public void Cargar(ref clsBeTbUsuarios oBeGruposEntidades, ref DataRow dr) {
		try {
				...
				oBeGruposEntidades.Statuss = ( IsDBNull(dr.Item['Statuss']) ? '' : dr.Item['Statuss'] )
				oBeGruposEntidades.NoEliminable = ( IsDBNull(dr.Item['NoEliminable']) ? False : dr.Item['NoEliminable'] )
				oBeGruposEntidades.FechaRegistro = ( IsDBNull(dr.Item['FechaRegistro']) ?  : dr.Item['FechaRegistro'] )
		catch (Exception ex) {
			throw ex;
		}
	}
	...
	public int Insertar(ref clsBeGruposEntidades oBeGruposEntidades) {
		try {
			...
			cmd.Parameters.Add(new SqlClient.SqlParameter("@STATUSS", oBeGruposEntidades.Statuss));
